...
This event monitors production system health and creates incidents when thresholds are exceeded. Runs every 15 minutes.
...
4. **Post to #incidents Slack channel:**
   ```
   🚨 [P2] Production Performance Degradation
   
   📊 Metric: API response time p95 = 1,234ms (threshold: 1,000ms)
   ⏰ Duration: 5 minutes
   🔗 PagerDuty: [incident link]
   📈 Grafana: [dashboard link]
   
   🔍 Likely cause: Database slow queries after deploy #1234
   ✅ Suggested action: Rollback deploy or add index to users table
   ```

### Recovery Notification

When metric returns to normal for 3 consecutive checks:
- Resolve PagerDuty incident automatically
